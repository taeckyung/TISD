...
def compute_self_distillation_loss(
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        if use_topk:
            ...
            if alpha == 0.0:
                per_token_loss = compute_topk_forward_kl(student_topk_log_probs, teacher_topk_log_probs, add_tail=add_tail)
            elif alpha == 1.0:
                per_token_loss = compute_topk_reverse_kl(student_topk_log_probs, teacher_topk_log_probs, add_tail=add_tail)
            else:
                per_token_loss = compute_topk_jsd(
                    student_topk_log_probs, teacher_topk_log_probs, alpha=alpha, add_tail=add_tail
                )
        else:
...
def topk_log_probs_add_tail(log_probs: torch.Tensor) -> torch.Tensor:
    """Append a tail bucket holding the residual probability mass (1 - sum of the top-k probabilities).

    Given top-k log-probs over the original (unnormalized) distribution, the tail bucket lets the
    truncated distribution sum to 1 without renormalizing the top-k entries.
    """
    log_s = torch.logsumexp(log_probs, dim=-1, keepdim=True)
    log_s = torch.clamp(log_s, max=-1e-7)  # avoid log_s >= 0, i.e., sum(probs) >= 1
    tail_log = torch.log(-torch.expm1(log_s))  # 1 - exp(x) = -(exp(x) - 1), computed stably by expm1
    return torch.cat([log_probs, tail_log], dim=-1)


def renorm_topk_log_probs(logp: torch.Tensor) -> torch.Tensor:
    """Renormalize top-k log-probs so the truncated support sums to 1."""
    logZ = torch.logsumexp(logp, dim=-1, keepdim=True)
    return logp - logZ


def normalize_topk_log_probs(log_probs: torch.Tensor, add_tail: bool) -> torch.Tensor:
    """Turn raw top-k log-probs into a proper distribution, with or without a tail bucket."""
    if add_tail:
        return topk_log_probs_add_tail(log_probs)
    return renorm_topk_log_probs(log_probs)
...
def compute_topk_reverse_kl(
    ...
    """Per-token reverse KL ``KL(pi_student || pi_teacher)`` from top-k log-probs on a shared support.

    Both inputs must be log-probs over the SAME support (the student's top-k token indices); obtain
    ``teacher_topk_log_probs`` by gathering teacher logits at the student's top-k indices.
    Shapes ``[..., k]`` -> returns ``[...]``.
    """
    student_log = normalize_topk_log_probs(student_topk_log_probs, add_tail)
    teacher_log = normalize_topk_log_probs(teacher_topk_log_probs, add_tail)
    return (torch.exp(student_log) * (student_log - teacher_log)).sum(-1)
...
def compute_topk_forward_kl(
    ...
    """Per-token forward KL ``KL(pi_teacher || pi_student)`` from top-k log-probs on a shared support."""
    student_log = normalize_topk_log_probs(student_topk_log_probs, add_tail)
    teacher_log = normalize_topk_log_probs(teacher_topk_log_probs, add_tail)
    return (torch.exp(teacher_log) * (teacher_log - student_log)).sum(-1)


def compute_topk_jsd(
    student_topk_log_probs: torch.Tensor,
    teacher_topk_log_probs: torch.Tensor,
    alpha: float = 0.5,
    add_tail: bool = True,
) -> torch.Tensor:
    """Per-token generalized Jensen-Shannon divergence from top-k log-probs on a shared support.

    ``alpha=0.5`` is the symmetric JSD; alpha interpolates between the student and teacher terms.
    """
    if not 0.0 <= alpha <= 1.0:
        raise ValueError(f"JSD alpha must be in [0, 1], got {alpha}")
    student_log = normalize_topk_log_probs(student_topk_log_probs, add_tail)
    teacher_log = normalize_topk_log_probs(teacher_topk_log_probs, add_tail)
    alpha_t = torch.tensor(alpha, dtype=student_log.dtype, device=student_log.device)
    mixture_log = torch.logsumexp(
        torch.stack([student_log + torch.log1p(-alpha_t), teacher_log + torch.log(alpha_t)]),
        dim=0,
    )
    kl_teacher = torch.exp(teacher_log) * (teacher_log - mixture_log)
    kl_student = torch.exp(student_log) * (student_log - mixture_log)
    return torch.lerp(kl_student, kl_teacher, alpha_t).sum(-1)
