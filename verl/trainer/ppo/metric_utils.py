...
def process_validation_metrics(
    ...
    various statistical measures including means, standard deviations, direct pass rates,
    best/worst values, and majority voting results. It also performs bootstrap sampling to
    estimate statistics for different sample sizes.
    ...
    Returns:
        ...
        - "pass@N": Whether any of the first N samples has a positive value, averaged across prompts
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        ...
        for uid, var2vals in uid2var2vals.items():
            ...
            for var_name, var_vals in var2vals.items():
                ...
                if n_resps == 1:
                    metric["pass@1"] = float(var_vals[0] > 0)
                else:
                    ...
                    for n in ns:
                        # Direct observed pass@N, matching the standalone math evaluator:
                        # a prompt passes when any of its first N responses has a positive score.
                        metric[f"pass@{n}"] = float(any(value > 0 for value in var_vals[:n]))
    ...
    # Conditional mean: mean score over problems that have at least one correct rollout (score > 0).
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if not var_vals or isinstance(var_vals[0], str):
                ...
                if any(v > 0 for v in var_vals):
                    n_resps = len(var_vals)
                    data_src2var2metric2uid_vals[data_source][var_name][f"cond_mean@{n_resps}"].append(float(np_mean(var_vals)))
