        ...
        for default_backend, logger_instance in self.logger.items():
            if backend is None or default_backend in backend:
                ...
                if default_backend == "wandb":
                    self._update_wandb_sciknoweval_val_max_summary(logger_instance, data)

    _SCIKNOWEVAL_VAL_MAX_PATTERNS = (
        re.compile(r"^val-core/sciknoweval/acc/mean@\d+$"),
        re.compile(r"^val-core/sciknoweval/acc/best@\d+/mean$"),
        re.compile(r"^val-core/sciknoweval/acc/pass@\d+$"),
    )

    def _update_wandb_sciknoweval_val_max_summary(self, wandb, data):
        """Maintain W&B summary maxes for SciKnowEval validation accuracy metrics."""
        if wandb.run is None:
            return

        for key, value in data.items():
            if not any(pattern.match(key) for pattern in self._SCIKNOWEVAL_VAL_MAX_PATTERNS):
                continue
            if isinstance(value, bool) or not isinstance(value, numbers.Real):
                continue

            value = float(value)
            summary_key = f"{key}/max"
            previous = wandb.run.summary.get(summary_key)
