...
def get_master_addr_port() -> tuple[str, str]:
    ...
    # Allow pinning an explicit port via env (set by the launch script if desired).
    override = os.environ.get("VERL_MASTER_PORT")
    if override:
        return addr, str(override)
    # Pick the rendezvous port from BELOW the ephemeral range (32768-60999) so the
    # job's own vLLM/Ray/NCCL ephemeral sockets can never grab it in the window
    # between selection here and torch.distributed binding it in the worker. Probe
    # with a plain bind (no SO_REUSEADDR) -- exactly how torch will bind it -- so a
    # port left in TIME_WAIT or held by a stale run is skipped, not collided with.
    for port in range(29500, 29600):
        try:
            ...
            continue
    # Fallback: original ephemeral behavior if the whole low range is occupied.
