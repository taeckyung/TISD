...
# Loss modes trained by distilling the privileged-context (EMA) teacher on the retained trajectory:
# SDPO on the original rollout, TRD on the teacher-refined rollout, TISD on the branched rollout.
SELF_DISTILLATION_LOSS_MODES = ("sdpo", "trd", "tisd")
...
class DataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def compute_branch_kl(self, data: DataProto) -> dict[str, torch.Tensor]:
        """Per-token student-teacher divergence used by TISD to select the branch position.

        Runs the current actor on the plain rollout sequence and the EMA teacher (``teacher_module``) on
        the privileged-context sequence (``teacher_*`` keys). The divergence is computed on the student's
        top-k support with a tail bucket, using ``self_distillation.branch_alpha`` (0.0 = forward KL,
        1.0 = reverse KL, otherwise generalized JSD). Also returns each model's top-1 token per position.

        Required ``data.batch`` keys: ``responses``, ``input_ids``, ``attention_mask``, ``position_ids``,
        ``teacher_input_ids``, ``teacher_attention_mask``, ``teacher_position_ids``.
        Required ``data.meta_info``: ``micro_batch_size``, ``temperature``, ``distill_topk``.
        """
        self.actor_module.eval()

        micro_batch_size = data.meta_info["micro_batch_size"]
        temperature = data.meta_info["temperature"]
        distill_topk = data.meta_info["distill_topk"]
        # Score and force the branch with the EMA teacher, i.e., the same distribution the student is
        # distilled toward. Falls back to the current actor if no separate teacher module exists.
        use_ema_teacher = bool(data.meta_info.get("use_ema_teacher", False))
        teacher_fwd_module = (
            self.teacher_module
            if use_ema_teacher and self.teacher_module is not None and self.teacher_module is not self.actor_module
            else None
        )
        self_distillation_config = getattr(self.config, "self_distillation", None)
        add_tail = bool(getattr(self_distillation_config, "distillation_add_tail", True))
        branch_alpha = float(getattr(self_distillation_config, "branch_alpha", 1.0))
        pad_token_id = data.meta_info.get("pad_token_id", 0)
        ...
        for micro_batch in micro_batches:
            ...
            with torch.no_grad():
                ...
                teacher_outputs = self._forward_micro_batch(
                    ...
                    module=teacher_fwd_module,
                )
                teacher_top1 = self._forward_micro_batch(
                    teacher_inputs,
                    temperature=temperature,
                    calculate_entropy=False,
                    distill_topk=1,
                ...
                teacher_argmax = teacher_top1["topk_indices"][..., 0]

                student_topk_logps = student_outputs["topk_logps"]
                teacher_topk_logps = teacher_outputs["topk_logps"]
                student_argmax = student_topk_indices[..., 0]
                if branch_alpha == 0.0:
                    kl = compute_topk_forward_kl(student_topk_logps, teacher_topk_logps, add_tail=add_tail)
                elif branch_alpha == 1.0:
                    kl = compute_topk_reverse_kl(student_topk_logps, teacher_topk_logps, add_tail=add_tail)
                else:
                    kl = compute_topk_jsd(student_topk_logps, teacher_topk_logps, alpha=branch_alpha, add_tail=add_tail)
            branch_kl_lst.append(kl)
            student_argmax_lst.append(student_argmax)
            teacher_argmax_lst.append(teacher_argmax)

        return {
            "branch_kl": torch.concat(branch_kl_lst, dim=0),
            "student_argmax_ids": torch.concat(student_argmax_lst, dim=0),
            "teacher_argmax_ids": torch.concat(teacher_argmax_lst, dim=0),
        }
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    # Teacher-distribution targets are only needed by the self-distillation objectives.
                    return_all_logps = (
