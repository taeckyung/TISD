...
class SelfDistillationConfig(BaseConfig):
    ...
    Args:
        Distillation is enabled when policy_loss.loss_mode is "sdpo", "trd", or "tisd".
        ...
        branch_alpha (float): TISD branch-selection divergence, independent of alpha (the learning
            objective). 0.0=forward KL, 1.0=reverse KL, in-between=JSD.
        branch_selection (str): TISD branch position. "argmax" (peak divergence), "first_divergence"
            (first teacher/student top-1 mismatch), or "random" (uniform response position).
        branch_token_source (str): TISD token forced at the branch position. "teacher" (teacher argmax)
            or "student" (the original sampled token; the suffix is still regenerated).
        regeneration_rounds (int): Total TISD branch-regenerate rounds per failed rollout. 1 is the
            standard TISD; larger values re-branch strictly after the previous branch position.
    """
    ...
    # ----- TISD (policy_loss.loss_mode == "tisd") -----
    branch_alpha: float = 1.0
    branch_selection: str = "argmax"
    branch_token_source: str = "teacher"
    regeneration_rounds: int = 1

    def __post_init__(self):
        if not 0.0 <= self.alpha <= 1.0:
            raise ValueError(f"self_distillation.alpha must be in [0,1], got {self.alpha}")
        if not 0.0 <= self.branch_alpha <= 1.0:
            raise ValueError(f"self_distillation.branch_alpha must be in [0,1], got {self.branch_alpha}")
        if self.branch_selection not in ("argmax", "first_divergence", "random"):
            raise ValueError(
                "self_distillation.branch_selection must be 'argmax', 'first_divergence', or 'random', "
                f"got {self.branch_selection}"
            )
        if self.branch_token_source not in ("teacher", "student"):
            raise ValueError(
                f"self_distillation.branch_token_source must be 'teacher' or 'student', got {self.branch_token_source}"
            )
        if self.regeneration_rounds < 1:
            raise ValueError(
                f"self_distillation.regeneration_rounds must be at least 1, got {self.regeneration_rounds}"
            )
