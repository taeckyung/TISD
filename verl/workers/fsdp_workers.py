...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _set_rollout_state_dict_type(self, module):
        """Configure `module`'s FSDP state-dict type to match the rollout weight-sync expectation
        (same logic applied to actor_module_fsdp at init), so teacher-source syncs hand vLLM the same
        format as actor-source syncs (full for single-rank fsdp1, sharded for multi-rank fsdp1)."""
    ...
    async def rollout_mode(self):
        ...
        When `self._rollout_weight_source == "teacher"`, sync the EMA teacher (ref_module_fsdp) into the
        rollout engine instead of the current actor. TRD uses this to generate the refined trajectory with
        the same EMA teacher it is distilled toward. The flag is set around that generation only and
        defaults to "actor", so the main rollout, TISD continuations, and validation use the current actor.
        """
        aggressive_empty_cache(force_sync=True)

        weight_source = getattr(self, "_rollout_weight_source", "actor")
        if weight_source == "teacher":
            src_module = self.ref_module_fsdp
            self._set_rollout_state_dict_type(src_module)
        else:
            src_module = self.actor_module_fsdp
        # Expect "teacher" only for TRD refinement and "actor" for all other generation.
        if torch.distributed.get_rank() == 0:
            logger.info(f"rollout_mode: syncing rollout engine from weight_source={weight_source}")

        log_gpu_memory_usage("Before load_fsdp_model_to_gpu", logger=logger)
        if self._is_offload_param:
            load_fsdp_model_to_gpu(src_module)
        log_gpu_memory_usage("After load_fsdp_model_to_gpu", logger=logger)

        peft_config = None
        peft_model = getattr(src_module, "_fsdp_wrapped_module", src_module)
        if hasattr(peft_model, "peft_config"):  # LoRA
            if weight_source == "teacher":
                raise ValueError("Generating with the EMA teacher is not supported with LoRA.")
            ...
            params = src_module.state_dict()

        params = convert_weight_keys(params, getattr(src_module, "_fsdp_wrapped_module", src_module))
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(src_module)
    ...
    def init_model(self):
        ...
        if self._is_ref:
            ...
            if self._is_actor:
                ...
                if self_distillation_cfg is not None and loss_mode in ("sdpo", "trd", "tisd"):
        ...
        if self._is_actor:
            ...
            if (
                ...
                and loss_mode in ("sdpo", "trd", "tisd")
    ...
    def set_rollout_weight_source(self, source: str):
        """Select which weights rollout_mode syncs into the engine on the next generate.

        "teacher" -> EMA teacher (ref_module_fsdp), used for TRD refinement; "actor" -> current actor.
        The trainer resets it to "actor" right after the refinement rollout."""
        assert source in ("actor", "teacher"), source
        self._rollout_weight_source = source
        return True
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_branch_kl")
    def compute_branch_kl(self, data: DataProto):
        """TISD branch scoring: per-token student-teacher divergence and top-1 tokens over responses."""
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)
        ...
        data.meta_info["temperature"] = self.config.rollout.temperature
        data.meta_info["distill_topk"] = self.config.actor.self_distillation.distillation_topk
        # Select the branch point t* and the forced token with the same teacher that is distilled
        # toward (teacher_module = EMA self). dp_actor falls back to the current actor only if no
        # separate teacher_module exists.
        data.meta_info["use_ema_teacher"] = True
        data.meta_info.setdefault("pad_token_id", self.tokenizer.pad_token_id)

        with self.ulysses_sharding_manager:
            outputs = self.actor.compute_branch_kl(data=data)
            output = DataProto.from_dict(
                tensors={
                    "branch_kl": outputs["branch_kl"],
                    "student_argmax_ids": outputs["student_argmax_ids"],
                    "teacher_argmax_ids": outputs["teacher_argmax_ids"],
                }
            )
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self.ema_teacher_checkpoint_manager is not None:
            ema_teacher_path = os.path.join(local_path, "ema_teacher")
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        if self.ema_teacher_checkpoint_manager is not None:
            ema_teacher_path = os.path.join(local_path, "ema_teacher")
            world_size = self.ema_teacher_checkpoint_manager.world_size
            teacher_shards_complete = os.path.isfile(os.path.join(ema_teacher_path, "fsdp_config.json")) and all(
                os.path.isfile(os.path.join(ema_teacher_path, f"model_world_size_{world_size}_rank_{rank}.pt"))
                for rank in range(world_size)
            )
            if teacher_shards_complete:
                ...
                log_with_rank(
                    "EMA teacher checkpoint is absent or incomplete; keeping the teacher initialized from "
                    "the configured reference model. New checkpoints will include ema_teacher/.",
                    rank=self.rank,
                    logger=logger,
                    log_only_rank_0=True,
                )
