#!/bin/bash
# SDPO baseline on SciKnowEval L3 without rich environment feedback (paper Table 2; Table 12).
#
...
#
# Usage: bash experiments/science/sdpo.sh [hydra overrides...]
#   DOMAINS="biology physics" bash experiments/science/sdpo.sh
...
for DOMAIN in $DOMAINS; do
    ...
    run_training "SDPO-sciknoweval-${DOMAIN}-${MODEL_NAME}-${RUN_ID}" sdpo "$DATA_PATH" \
        $(common_args) \
